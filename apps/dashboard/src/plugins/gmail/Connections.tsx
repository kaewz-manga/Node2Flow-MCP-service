...
import {
  ...
  Mail, Lock, Tag, MoreHorizontal, Pencil, Info,
} from 'lucide-react';

const PRODUCT_TYPE = 'gmail';
...
export default function Connections() {
  ...
  const handleAddConnection = async (e: React.FormEvent) => {
    ...
    if (!formClientId.trim() || !formClientSecret.trim() || !formRefreshToken.trim()) {
      setFormError('All OAuth fields are required');
    ...
    if (res.success && res.data) {
      ...
      setFormName(''); setFormClientId(''); setFormClientSecret(''); setFormRefreshToken('');
      fetchConnections();
    } else {
  ...
  return (
    <div className="space-y-6">
      {/* MCP Endpoint */}
      <Item>
        <ItemMedia><img src="/logos/gmail.svg?v=2" alt="Gmail" className="h-10 w-10" /></ItemMedia>
        <ItemContent>
      ...
      {show2faAlert && (
        <Alert>
          ...
          <AlertDescription>
            Two-factor authentication protects your Gmail connections. Enable it in Settings.
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">Gmail Connections</h2>
          <p className="text-sm text-muted-foreground">Manage your Gmail OAuth connections</p>
        </div>
      ...
      {connections.length === 0 && !error && (
        <Empty>
          <EmptyHeader>
            <EmptyMedia><Mail className="h-10 w-10 text-muted-foreground" /></EmptyMedia>
            <EmptyTitle>No Gmail connections</EmptyTitle>
            <EmptyDescription>Add your Gmail OAuth credentials to get started.</EmptyDescription>
          </EmptyHeader>
          <EmptyContent>
            <Button onClick={() => setShowAddModal(true)} className="bg-green-600 hover:bg-green-700">
              <Plus className="h-4 w-4 mr-2" /> Add Connection
      ...
      <AddModal open={showAddModal} onOpenChange={setShowAddModal}>
        <AddModalContent>
          <AddModalHeader>
            <AddModalTitle>Add Gmail Connection</AddModalTitle>
          </AddModalHeader>
          <form onSubmit={handleAddConnection} className="space-y-4 py-4">
            ...
            <Field>
              ...
              <InputGroup>
                ...
                <InputGroupInput placeholder="My Gmail" value={formName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormName(e.target.value)} required />
              </InputGroup>
            </Field>
            <Field>
              <FieldLabel>Client ID</FieldLabel>
              <FieldDescription>From Google Cloud Console &gt; Credentials</FieldDescription>
              <InputGroup>
                <InputGroupAddon><Mail className="h-4 w-4" /></InputGroupAddon>
                <InputGroupInput placeholder="xxxx.apps.googleusercontent.com" value={formClientId} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormClientId(e.target.value)} required />
              </InputGroup>
            </Field>
            <Field>
              ...
              <InputGroup>
                ...
                <InputGroupInput type="password" placeholder="GOCSPX-..." value={formClientSecret} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormClientSecret(e.target.value)} required />
              </InputGroup>
            </Field>
            <Field>
              <FieldLabel>Refresh Token</FieldLabel>
              <FieldDescription>Generated via OAuth Playground with Gmail scopes</FieldDescription>
              <InputGroup>
                <InputGroupAddon><Key className="h-4 w-4" /></InputGroupAddon>
                <InputGroupInput type="password" placeholder="1//0..." value={formRefreshToken} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormRefreshToken(e.target.value)} required />
              </InputGroup>
            </Field>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={() => setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this Gmail connection and its API key.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
