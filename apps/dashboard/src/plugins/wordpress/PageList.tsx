          ...
          {pages.map((page) => (
            <Card key={page.id} className="overflow-hidden hover:shadow-md transition-all">
              <div className="flex items-center gap-3 px-4 py-3 hover:bg-muted">
                ...
                <Badge variant="secondary" className={page.status === 'publish' ? 'bg-green-900/30 text-green-400' : page.status === 'draft' ? 'bg-amber-900/30 text-amber-400' : 'bg-muted text-foreground'}>
                  {page.status}
                </Badge>
