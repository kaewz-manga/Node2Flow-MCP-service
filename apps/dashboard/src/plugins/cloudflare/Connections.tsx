...
import {
  ...
  Cloud, Lock, Tag, MoreHorizontal, Pencil, Info, Hash,
} from 'lucide-react';

const PRODUCT_TYPE = 'cloudflare';
...
export default function Connections() {
  ...
  const handleAddConnection = async (e: React.FormEvent) => {
    ...
    const config: Record<string, unknown> = {
      api_token: formApiToken.trim(),
    };
    if (formAccountId.trim()) {
      config.account_id = formAccountId.trim();
    }
    ...
    if (res.success && res.data) {
      ...
      setFormName(''); setFormApiToken(''); setFormAccountId('');
      fetchConnections();
    } else {
    ...
    <div className="space-y-6">
      {show2faAlert && (
        <Alert>
          ...
          <AlertDescription>
            Two-factor authentication protects your Cloudflare connections. Enable it in Settings.
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">Cloudflare Connections</h2>
          <p className="text-sm text-muted-foreground">Manage your Cloudflare API Token connections</p>
        </div>
      ...
      {connections.length === 0 && !error && (
        <Empty>
          <EmptyHeader>
            <EmptyMedia><Cloud className="h-10 w-10 text-muted-foreground" /></EmptyMedia>
            <EmptyTitle>No Cloudflare connections</EmptyTitle>
            <EmptyDescription>Add your Cloudflare API Token to get started.</EmptyDescription>
          </EmptyHeader>
          <EmptyContent>
            <Button onClick={() => setShowAddModal(true)} className="bg-green-600 hover:bg-green-700">
              <Plus className="h-4 w-4 mr-2" /> Add Connection
      ...
      <AddModal open={showAddModal} onOpenChange={setShowAddModal}>
        <AddModalContent>
          <AddModalHeader>
            <AddModalTitle>Add Cloudflare Connection</AddModalTitle>
          </AddModalHeader>
          <form onSubmit={handleAddConnection} className="space-y-4 py-4">
            ...
            <Field>
              ...
              <InputGroup>
                ...
                <InputGroupInput placeholder="My Cloudflare" value={formName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormName(e.target.value)} required />
              </InputGroup>
            </Field>
            <Field>
              <FieldLabel>API Token</FieldLabel>
              <FieldDescription>From Cloudflare Dashboard &gt; My Profile &gt; API Tokens</FieldDescription>
              <InputGroup>
                <InputGroupAddon><Key className="h-4 w-4" /></InputGroupAddon>
                <InputGroupInput type="password" placeholder="Bearer token value" value={formApiToken} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormApiToken(e.target.value)} required />
              </InputGroup>
            </Field>
            <Field>
              <FieldLabel>Account ID <span className="text-muted-foreground font-normal">(optional)</span></FieldLabel>
              <FieldDescription>Found in Cloudflare Dashboard sidebar. Can set later with cf_set_active_account.</FieldDescription>
              <InputGroup>
                <InputGroupAddon><Hash className="h-4 w-4" /></InputGroupAddon>
                <InputGroupInput placeholder="e.g. ed77f292a2c8173c..." value={formAccountId} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormAccountId(e.target.value)} />
              </InputGroup>
            </Field>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={() => setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this Cloudflare connection and its API key.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
