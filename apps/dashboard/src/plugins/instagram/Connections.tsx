...
import {
  ...
  Camera, Lock, Tag, MoreHorizontal, Pencil, Info, Hash,
} from 'lucide-react';

const PRODUCT_TYPE = 'instagram';
...
export default function Connections() {
  ...
  const handleAddConnection = async (e: React.FormEvent) => {
    ...
    if (res.success && res.data) {
      ...
      setFormName(''); setFormAccessToken(''); setFormAccountId('');
      fetchConnections();
    } else {
  ...
  return (
    <div className="space-y-6">
      <Item>
        <ItemMedia><img src="/logos/instagram.svg?v=2" alt="Instagram" className="h-10 w-10" /></ItemMedia>
        <ItemContent>
      ...
      {show2faAlert && (
        <Alert>
          ...
          <AlertDescription>
            Two-factor authentication protects your Instagram connections. Enable it in Settings.
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">Instagram Connections</h2>
          <p className="text-sm text-muted-foreground">Manage your Instagram connections</p>
        </div>
      ...
      {connections.length === 0 && !error && (
        <Empty>
          <EmptyHeader>
            <EmptyMedia><Camera className="h-10 w-10 text-muted-foreground" /></EmptyMedia>
            <EmptyTitle>No Instagram connections</EmptyTitle>
            <EmptyDescription>Add your Instagram Access Token to get started.</EmptyDescription>
          </EmptyHeader>
          <EmptyContent>
            <Button onClick={() => setShowAddModal(true)} className="bg-green-600 hover:bg-green-700">
              <Plus className="h-4 w-4 mr-2" /> Add Connection
      ...
      <AddModal open={showAddModal} onOpenChange={setShowAddModal}>
        <AddModalContent>
          <AddModalHeader>
            <AddModalTitle>Add Instagram Connection</AddModalTitle>
          </AddModalHeader>
          <form onSubmit={handleAddConnection} className="space-y-4 py-4">
            ...
            <Field>
              ...
              <InputGroup>
                ...
                <InputGroupInput placeholder="My Instagram Account" value={formName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormName(e.target.value)} required />
              </InputGroup>
            </Field>
            <Field>
              <FieldLabel>Access Token</FieldLabel>
              <FieldDescription>Long-lived User Access Token with Instagram permissions</FieldDescription>
              <InputGroup>
                <InputGroupAddon><Lock className="h-4 w-4" /></InputGroupAddon>
                <InputGroupInput type="password" placeholder="EAAxxxxxxxxx..." value={formAccessToken} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormAccessToken(e.target.value)} required />
              </InputGroup>
            </Field>
            <Field>
              <FieldLabel>Account ID <span className="text-muted-foreground font-normal">(Optional)</span></FieldLabel>
              <FieldDescription>Instagram Business Account ID — found via /me/accounts endpoint</FieldDescription>
              <InputGroup>
                <InputGroupAddon><Hash className="h-4 w-4" /></InputGroupAddon>
                <InputGroupInput placeholder="17841400123456789" value={formAccountId} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormAccountId(e.target.value)} />
              </InputGroup>
            </Field>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={() => setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this Instagram connection and its API key.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
