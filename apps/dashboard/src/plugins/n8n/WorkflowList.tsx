        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <Card className="bg-gradient-to-t from-primary/5 to-card shadow-sm">
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-2xl font-semibold tabular-nums text-primary">{workflows.filter(w => w.active).length}</CardTitle>
            </CardHeader>
            <CardFooter className="text-sm text-muted-foreground">
          ...
          {workflows.map((wf) => (
            <Card key={wf.id} className="overflow-hidden hover:shadow-md transition-all">
              ...
              <div className="flex items-center gap-3 px-4 py-3 hover:bg-muted">
                ...
                <Button variant="ghost" size="icon" className="h-7 w-7 text-primary hover:bg-primary/10" onClick={() => handleExecute(wf.id)} disabled={executing === wf.id} title="Execute">
                  ...
                  {detailLoading ? (
                    ...
                    <>
                        ...
                        <CardContent className="p-3">
                          ...
                          {editingTags ? (
                            <div className="space-y-2">
                              ...
                              <div className="flex gap-2">
                                <Button size="sm" className="" onClick={handleSaveTags} disabled={savingTags}>
                      ...
                      <div className="flex gap-2 flex-wrap">
                        <Button variant="outline" size="sm" className={detail.active ? 'text-amber-400 border-amber-700 hover:bg-amber-900/30' : 'text-primary border-primary/50 hover:bg-primary/10'} onClick={() => handleToggle(detail)}>
                          {detail.active ? <PowerOff className="h-3.5 w-3.5 mr-2" /> : <Power className="h-3.5 w-3.5 mr-2" />}
                          {detail.active ? 'Deactivate' : 'Activate'}
                        </Button>
                        <Button size="sm" className="" onClick={() => handleExecute(detail.id)} disabled={executing === detail.id}>
