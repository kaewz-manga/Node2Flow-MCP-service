      ...
      {!loading && (
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <Card className="bg-gradient-to-t from-green-500/5 to-card shadow-sm">
            ...
            <CardFooter className="text-sm text-muted-foreground">
              <Tag className="h-3.5 w-3.5 mr-1.5 text-green-500" />
              Most recently added
            </CardFooter>
          </Card>
                ...
                {editingId === tag.id ? (
                  <div className="flex-1 flex gap-2">
                    <Input
                      ...
                      autoFocus
                    />
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-green-400 hover:bg-green-900/30" onClick={() => handleUpdate(tag.id)}><Check className="h-4 w-4" /></Button>
