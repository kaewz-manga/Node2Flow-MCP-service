...
import {
  ...
  FileText, Lock, Tag, MoreHorizontal, Pencil, Info, Hash,
} from 'lucide-react';

const PRODUCT_TYPE = 'facebook-pages';
...
export default function Connections() {
  ...
  const handleAddConnection = async (e: React.FormEvent) => {
    ...
    if (!formPageAccessToken.trim()) {
      setFormError('Page Access Token is required');
      setFormLoading(false);
      return;
    }

    const config: Record<string, unknown> = {
      page_access_token: formPageAccessToken.trim(),
    };
    if (formPageId.trim()) {
      config.page_id = formPageId.trim();
    }
    ...
    if (res.success && res.data) {
      ...
      setFormName(''); setFormPageAccessToken(''); setFormPageId('');
      fetchConnections();
    } else {
    ...
    <div className="space-y-6">
      {show2faAlert && (
        <Alert>
          ...
          <AlertDescription>
            Two-factor authentication protects your Facebook Page connections. Enable it in Settings.
          </AlertDescription>
        </Alert>
      )}

      <Item>
        <ItemMedia><img src="/logos/facebook.svg?v=2" alt="Facebook Pages" className="h-10 w-10" /></ItemMedia>
        <ItemContent>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">Facebook Page Connections</h2>
          <p className="text-sm text-muted-foreground">Manage your Facebook Page connections</p>
        </div>
      ...
      {connections.length === 0 && !error && (
        <Empty>
          <EmptyHeader>
            <EmptyMedia><FileText className="h-10 w-10 text-muted-foreground" /></EmptyMedia>
            <EmptyTitle>No Facebook Page connections</EmptyTitle>
            <EmptyDescription>Add your Page Access Token to get started.</EmptyDescription>
          </EmptyHeader>
          <EmptyContent>
            <Button onClick={() => setShowAddModal(true)} className="bg-green-600 hover:bg-green-700">
              <Plus className="h-4 w-4 mr-2" /> Add Connection
      ...
      <AddModal open={showAddModal} onOpenChange={setShowAddModal}>
        <AddModalContent>
          <AddModalHeader>
            <AddModalTitle>Add Facebook Page Connection</AddModalTitle>
          </AddModalHeader>
          <form onSubmit={handleAddConnection} className="space-y-4 py-4">
            ...
            <Field>
              ...
              <InputGroup>
                ...
                <InputGroupInput placeholder="My Facebook Page" value={formName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormName(e.target.value)} required />
              </InputGroup>
            </Field>
            <Field>
              <FieldLabel>Page Access Token</FieldLabel>
              <FieldDescription>Long-lived Page Access Token from your Facebook App</FieldDescription>
              <InputGroup>
                <InputGroupAddon><Lock className="h-4 w-4" /></InputGroupAddon>
                <InputGroupInput type="password" placeholder="EAAxxxxxxxxx..." value={formPageAccessToken} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormPageAccessToken(e.target.value)} required />
              </InputGroup>
            </Field>
            <Field>
              <FieldLabel>Page ID <span className="text-muted-foreground font-normal">(Optional)</span></FieldLabel>
              <FieldDescription>Auto-detected from token if omitted</FieldDescription>
              <InputGroup>
                <InputGroupAddon><Hash className="h-4 w-4" /></InputGroupAddon>
                <InputGroupInput placeholder="123456789012345" value={formPageId} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormPageId(e.target.value)} />
              </InputGroup>
            </Field>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={() => setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this Facebook Page connection and its API key.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
