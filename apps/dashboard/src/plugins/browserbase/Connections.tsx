...
import {
  ...
  Globe, Lock, Tag, MoreHorizontal, Pencil, Info, Hash,
} from 'lucide-react';

const PRODUCT_TYPE = 'browserbase';
...
export default function Connections() {
  ...
  const handleAddConnection = async (e: React.FormEvent) => {
    ...
    if (formGeminiKey.trim()) {
      config.gemini_api_key = formGeminiKey.trim();
    }
    ...
    if (res.success && res.data) {
      ...
      setFormName(''); setFormApiKey(''); setFormProjectId(''); setFormGeminiKey('');
      fetchConnections();
    } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">Browserbase Connections</h2>
          <p className="text-sm text-muted-foreground">Manage your Browserbase cloud browser connections</p>
        </div>
      ...
      {connections.length === 0 && !error && (
        <Empty>
          <EmptyHeader>
            <EmptyMedia><Globe className="h-10 w-10 text-muted-foreground" /></EmptyMedia>
            <EmptyTitle>No Browserbase connections</EmptyTitle>
            <EmptyDescription>Add your Browserbase API Key and Project ID to get started.</EmptyDescription>
          </EmptyHeader>
          <EmptyContent>
            <Button onClick={() => setShowAddModal(true)} className="bg-green-600 hover:bg-green-700">
              <Plus className="h-4 w-4 mr-2" /> Add Connection
      ...
      <AddModal open={showAddModal} onOpenChange={setShowAddModal}>
        <AddModalContent>
          <AddModalHeader>
            <AddModalTitle>Add Browserbase Connection</AddModalTitle>
          </AddModalHeader>
          <form onSubmit={handleAddConnection} className="space-y-4 py-4">
            ...
            <Field>
              ...
              <InputGroup>
                ...
                <InputGroupInput placeholder="My Browserbase" value={formName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormName(e.target.value)} required />
              </InputGroup>
            </Field>
            <Field>
              <FieldLabel>API Key</FieldLabel>
              <FieldDescription>From your Browserbase dashboard settings</FieldDescription>
              <InputGroup>
                <InputGroupAddon><Key className="h-4 w-4" /></InputGroupAddon>
                <InputGroupInput type="password" placeholder="bb_live_xxxxxxxxxxxx" value={formApiKey} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormApiKey(e.target.value)} required />
              </InputGroup>
            </Field>
            <Field>
              <FieldLabel>Project ID</FieldLabel>
              <FieldDescription>From your Browserbase project settings</FieldDescription>
              <InputGroup>
                <InputGroupAddon><Hash className="h-4 w-4" /></InputGroupAddon>
                <InputGroupInput placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" value={formProjectId} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormProjectId(e.target.value)} required />
              </InputGroup>
            </Field>
            <Field>
              <FieldLabel>Gemini API Key <span className="text-muted-foreground font-normal">(Optional)</span></FieldLabel>
              <FieldDescription>Required only for the autonomous agent tool (bb_agent)</FieldDescription>
              <InputGroup>
                <InputGroupAddon><Lock className="h-4 w-4" /></InputGroupAddon>
                <InputGroupInput type="password" placeholder="AIzaSy..." value={formGeminiKey} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormGeminiKey(e.target.value)} />
              </InputGroup>
            </Field>
            <Alert>
              <Info className="h-4 w-4" />
              <AlertDescription>
                Browserbase provides cloud browsers for automation. Sessions are isolated and secure.
              </AlertDescription>
            </Alert>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={() => setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this Browserbase connection and its API key.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
