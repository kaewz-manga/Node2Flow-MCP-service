...
import {
  ...
  GitBranch, Lock, Tag, MoreHorizontal, Pencil, Info,
} from 'lucide-react';

const PRODUCT_TYPE = 'github';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">GitHub Connections</h2>
          <p className="text-sm text-muted-foreground">Manage your GitHub PAT connections</p>
        </div>
      ...
      {connections.length === 0 && !error && (
        <Empty>
          <EmptyHeader>
            <EmptyMedia><GitBranch className="h-10 w-10 text-muted-foreground" /></EmptyMedia>
            <EmptyTitle>No GitHub connections</EmptyTitle>
            <EmptyDescription>Add your GitHub Personal Access Token to get started.</EmptyDescription>
          </EmptyHeader>
          <EmptyContent>
            <Button onClick={() => setShowAddModal(true)} className="bg-green-600 hover:bg-green-700">
              <Plus className="h-4 w-4 mr-2" /> Add Connection
      ...
      <AddModal open={showAddModal} onOpenChange={setShowAddModal}>
        <AddModalContent>
          <AddModalHeader>
            <AddModalTitle>Add GitHub Connection</AddModalTitle>
          </AddModalHeader>
          <form onSubmit={handleAddConnection} className="space-y-4 py-4">
            ...
            <Field>
              ...
              <InputGroup>
                ...
                <InputGroupInput placeholder="My GitHub" value={formName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormName(e.target.value)} required />
              </InputGroup>
            </Field>
            <Field>
              <FieldLabel>Personal Access Token</FieldLabel>
              <FieldDescription>From GitHub Settings &gt; Developer Settings &gt; Personal Access Tokens</FieldDescription>
              <InputGroup>
                <InputGroupAddon><Key className="h-4 w-4" /></InputGroupAddon>
                <InputGroupInput type="password" placeholder="ghp_xxxxxxxxxxxx or github_pat_xxxx" value={formToken} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormToken(e.target.value)} required />
              </InputGroup>
            </Field>
            <Alert>
              <Info className="h-4 w-4" />
              <AlertDescription>
                Use <strong>fine-grained tokens</strong> for better security. Grant only the permissions you need.
              </AlertDescription>
            </Alert>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={() => setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will permanently delete this GitHub connection and its API key.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
