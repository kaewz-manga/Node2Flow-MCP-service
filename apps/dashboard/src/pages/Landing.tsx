        ...
        <section className="py-16 border-b border-border bg-gradient-to-b from-background to-card/50">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <Card className="bg-gradient-to-t from-green-500/5 to-card shadow-sm text-center">
                <CardContent className="p-6">
    ...
    <Card className={`relative ${isPopular ? 'border-primary border-2 shadow-lg scale-105' : ''}`}>
      <CardContent className="p-6">
        ...
        <ul className="space-y-3 mb-6">
          <li className="flex items-center gap-2 text-sm text-muted-foreground"><Check className="h-4 w-4 text-green-400" />{isEnterprise ? 'Custom' : 'Unlimited'} service connections</li>
          <li className="flex items-center gap-2 text-sm text-muted-foreground"><Check className="h-4 w-4 text-green-400" />{isEnterprise ? 'Custom rate limits' : <span>{isMinuteUnlimited ? 'Unlimited' : minuteLimit} req/min{features.fair_use && <span className="text-muted-foreground"> (fair use)</span>}</span>}</li>
          <li className="flex items-center gap-2 text-sm text-muted-foreground"><Check className="h-4 w-4 text-green-400" />{isEnterprise ? 'Custom daily quota' : isDailyUnlimited ? <span className="text-primary font-semibold">Unlimited req/day</span> : <span>{dailyLimit.toLocaleString()} req/day</span>}</li>
          {features.analytics && <li className="flex items-center gap-2 text-sm text-muted-foreground"><Check className="h-4 w-4 text-green-400" />Usage analytics</li>}
          {features.support && <li className="flex items-center gap-2 text-sm text-muted-foreground"><Check className="h-4 w-4 text-green-400" />{features.support.charAt(0).toUpperCase() + features.support.slice(1)} support</li>}
          {features.private_server && <li className="flex items-center gap-2 text-sm text-muted-foreground"><Check className="h-4 w-4 text-green-400" />Private MCP server</li>}
        </ul>
        {isFree ? (
